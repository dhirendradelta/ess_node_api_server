...
var contentCtrl = {
	getNewsIt: function(countryid, depttid, cb){
		var query = `select * from content where content_type = ? and deptt_id = ? and countryid = ? and publish_status = 1 and status = 1`;
		return db.query(query, [2, depttid, countryid], cb);
	},
        getPolicy: function(countryid, depttid, cb){
		var query = `select * from content where content_type = ? and deptt_id = ? and countryid = ? and publish_status = 1 and status = 1`;
	...
	getAnnouncement: function(countryid, depttid, cb){
		var query = `select * from content where content_type = ? and deptt_id = ? and countryid = ? and publish_status = 1 and status = 1`;
		//console.log(query);
		return db.query(query, [1, depttid, countryid], cb);
	},
...
where content.status = 1`
	...
	updateContent:function(objContent, callback){
		...
		return db.query("UPDATE content SET update_date = now(), content_type = ?, countryid = ?, title = ?, short_desc =? , long_desc = ?, content_image = ?, update_author_id = ?, deptt_id=? where content_id = ?",[content_type, countryid, title, short_desc, long_desc, content_image, update_author_id, deptt_id, contentid],callback);
	},
	publishContent: function(content_id, status,update_author_id, callback) {
		return db.query("UPDATE content SET update_date = now(), publish_status = ?, update_author_id = ? where content_id = ?",[status, update_author_id, content_id],callback);		
	},
	deleteContent: function(content_id, update_author_id,callback) {
		return db.query("UPDATE content SET update_date = now(), status = ?, update_author_id = ? where content_id = ?",[0, update_author_id, content_id],callback);		
	}
};
